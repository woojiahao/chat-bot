...
public class AddGreetings extends TemplateResponse {
	public AddGreetings (String title, String label1Text) {
		super(title, label1Text);
	}

	@Override
	public void addTriggered (ActionEvent event) {
		// open greetings.txt with PrintWriter
		try (PrintWriter pw = new PrintWriter(
				new FileWriter(Const.GREETINGS_FILENAME, true))) {
			// append is set to true as we want to add on to the file
			// not replace its contents
			// check if input is empty
			if (!isEmpty()) {
				// if not empty, add input to file
				pw.print("\n" + TemplateResponse.getText());
			} else {
				// else prompt the user to enter something
				JOptionPane.showMessageDialog(
						null, "Please enter a greeting",
